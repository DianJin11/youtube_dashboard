"""
Created on Sept 6 22:28:40 2022
...
import pandas as pd
import numpy as np
import plotly.graph_objects as go  # highly customizable
import plotly.express as px  # user-friendly
...
# load data
def load_data():
    df_agg = pd.read_csv('Aggregated_Metrics_By_Video.csv').iloc[1:, :]  # integer location
    ...
    df_agg['Video publish time'] = pd.to_datetime(df_agg['Video publish time'])  # rename the columns
...
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months=12)  # focus on data within 12 mo
...
# look at the percentage difference
...
if add_sidebar == 'Aggregate Metrics':
    ...
    # get data information; trim to relevant data
...
if add_sidebar == 'Individual Video Analysis':
    # st.write('Ind')
    videos = tuple(df_agg['Video title'])  # non-iterable list
